...
{
    if (start == end)
        return void(tree[node] = pair<int, int>(val, 1));
    int mid = start + end >> 1;
    idx <= mid ? update(idx, val, node * 2, start, mid) : update(idx, val, node * 2 + 1, mid + 1, end);
    if (tree[node * 2].first == tree[node * 2 + 1].first)
        tree[node] = pair<int, int>(tree[node * 2].first, tree[node * 2].second + tree[node * 2 + 1].second);
    else
...
{
    if (l > end || r < start)
        return pair<int, int>(LLONG_MAX, 1);
    if (l <= start && end <= r)
        return tree[node];
    int mid = start + end >> 1;
    ...
    if (left.first == right.first)
        return pair<int, int>(left.first, left.second + right.second);
